...
@section('title', trans('admin.tests-solf.actions.index'))

@section('body')

    <tests-solf-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/tests-solves') }}'"
        inline-template>
        ...
        <div>
                    ...
                    <div>
                        <form @if(auth('admin')->user()->language === 'ar') dir="rtl" @endif class="@if(auth('admin')->user()->language === 'ar') rtl @endif form-horizontal form-create" method="post" @submit.prevent="onSubmit" :action="action" novalidate>
                            <div class="modal-header">
                                <div class="modal-title"><i class="icon-plus"></i> {{ trans('admin.tests-solf.actions.index') }}</div>
                            </div>
                                    ...
                                    <div class="dropdown">
                                         ...
                                         <div class="dropdown-menu dropdown-menu-right">
                                                                                             <button class="dropdown-item d-flex align-items-center" :class="{'active': (clickedBulkItemsCount > 0) || isClickedAll}"  v-validate="''" data-vv-name="enabled"  @click="onBulkItemsClickedAllWithPagination()"><i class="icon-check p-2"></i> {{ __('Select All') }}</button>
                                                                                                                                                                                              <button class="dropdown-item d-flex align-items-center" is='sortable' :column="'id'">{{ trans('admin.tests-solf.columns.id') }}</button>
                                                                                                                                                                                                    <button class="dropdown-item d-flex align-items-center" is='sortable' :column="'test_id'">{{ trans('admin.tests-solf.columns.test_id') }}</button>
                                                                                                                                                                                                    <button class="dropdown-item d-flex align-items-center" is='sortable' :column="'user_id'">{{ trans('admin.tests-solf.columns.user_id') }}</button>
                                                                                                                                                                                                    <button class="dropdown-item d-flex align-items-center" is='sortable' :column="'grade'">{{ trans('admin.tests-solf.columns.grade') }}</button>
                                                                                                                                       </div>
                                     </div>
                                    ...
                                    <div class="dropdown" v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                         ...
                                         <div class="dropdown-menu dropdown-menu-right">
                                             <button v-if='clickedBulkItemsCount < 2' @click="edit(editItem)" class="dropdown-item d-flex align-items-center"><i class="icon-pencil p-2"></i> {{ __('Edit') }}</button>
                                                                                             <button  @click="onBulkItemsClickedAll('/admin/tests-solves')" v-if="(clickedBulkItemsCount < pagination.state.total)" class="dropdown-item d-flex align-items-center"><i class="icon-check p-2"></i> {{ __('Select All Items') }}</button>
                                                ...
                                                <button @click="bulkDelete('/admin/tests-solves/bulk-destroy')" class="dropdown-item d-flex align-items-center"><i class="icon-trash p-2"></i> {{ __('Delete All') }}</button>
                                                                                     </div>
                                     </div>
                                </span>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            <p class="card-title m-0"><i class="icon-list"></i> {{ trans('admin.tests-solf.actions.index') }}</p>
                        </div>
                        <div class="col-auto">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-left">
                                    <a class="dropdown-item d-flex align-items-center" @click="addNew()" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.tests-solf.actions.create') }}</a>
                                                                            <a class="dropdown-item d-flex align-items-center" href="{{ url('admin/tests-solves/export') }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{ trans('admin.tests-solf.actions.export') }}</a>
                                                                    </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body p-0"  v-cloak>
                    <div class="card-block">
                        ...
                        <div class="p-5 text-center" v-if="!collection.length > 0">
                            ...
                            <button @click="addNew()" class="btn btn-primary" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.tests-solf.actions.create') }}</button>
                        </div>
                    </div>
                </div>
            </div>
