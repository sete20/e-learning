...
class UsersController extends Controller
{
    ...
    public function index(IndexUser $request)
    {
        ...
        $data = AdminListing::create(User::class)->processRequestAndGet(
            ...
            ['id', 'name', 'email', 'phone', 'other_phone', 'type', 'class_id', 'image'],

            // set columns to searchIn
            ['id', 'name', 'email', 'phone', 'other_phone', 'image', 'type']
        );

        foreach ($data as $item){
            if($item->type === "f"){
                $item->type = ["id"=>"f", "name"=>__('Female')];
            }
            else {
                $item->type = ["id"=>"m", "name"=>__('Male')];
            }
            $item->class_id = Classes::find($item->class_id);
