        ...
        <div>
                    ...
                    <div>
                        <form {{'@'}}if(auth('admin')->user()->language === 'ar') dir="rtl" {{'@'}}endif class="{{'@'}}if(auth('admin')->user()->language === 'ar') rtl {{'@'}}endif form-horizontal form-create" method="post" @submit.prevent="onSubmit" :action="action" novalidate>
                            <div class="modal-header">
                                <div class="modal-title"><i class="icon-plus"></i> {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.index') }}</div>
                            </div>

                            <div class="modal-body">
                                {{'@'}}include('admin.{{ $modelLangFormat }}.components.form-elements')
                            </div>
                            ...
                            <span class="input-group-prepend">
                                         ...
                                         <div class="dropdown-menu dropdown-menu-right">
                                            ...
                                            @foreach($columns as $col)
                                                 @if($col['name'] === 'published_at')
                                                     <button class="dropdown-item d-flex align-items-center" is='sortable' class="text-center" :column="'{{ $col['name'] }}'">{{'{{'}} trans('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</button>
                                                 @else
                                                     <button class="dropdown-item d-flex align-items-center" is='sortable' :column="'{{ $col['name'] }}'">{{'{{'}} trans('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</button>
                                                 @endif
                                             @endforeach
                                         </div>
                                     </div>
                                    ...
                                    <div class="dropdown" v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                         ...
                                         <div class="dropdown-menu dropdown-menu-right">
                                             ...
                                             @if(!$withoutBulk)
                                                <button  @click="onBulkItemsClickedAll('/admin/{{ $resource }}')" v-if="(clickedBulkItemsCount < pagination.state.total)" class="dropdown-item d-flex align-items-center"><i class="icon-check p-2"></i> {{'{{'}} __('Select All Items') }}</button>
                                                ...
                                                <button @click="bulkDelete('/admin/{{ $resource }}/bulk-destroy')" class="dropdown-item d-flex align-items-center"><i class="icon-trash p-2"></i> {{'{{'}} __('Delete All') }}</button>
                                            @endif
                                         </div>
                                     </div>
                                </span>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            <p class="card-title m-0"><i class="icon-list"></i> {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.index') }}</p>
                        </div>
                        <div class="col-auto">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-left">
                                    <a class="dropdown-item d-flex align-items-center" @click="addNew()" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.create') }}</a>
                                    @if($export)
                                        <a class="dropdown-item d-flex align-items-center" href="{{'{{'}} url('admin/{{ $resource }}/export') }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.export') }}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                <div class="card-body p-0"  v-cloak>
                    <div class="card-block">
                        ...
                        <div class="p-5 text-center" v-if="!collection.length > 0">
                            ...
                            <button @click="addNew()" class="btn btn-primary" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.create') }}</button>
                        </div>
                    </div>
                </div>
            </div>
